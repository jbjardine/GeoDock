...
capture_forensics(){
  ...
  [ -n "${source_dir}" ] && [ -f "${source_dir}/runner.log" ] && cp "${source_dir}/runner.log" "${FORENSICS_DIR}/runner.log" || true
  ...
  [ -n "${proxy_cid:-}" ] && docker inspect "${proxy_cid}" > "${FORENSICS_DIR}/proxy-inspect.json" 2>&1 || true
